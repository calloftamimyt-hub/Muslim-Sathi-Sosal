...
import { 
  ...
  ShieldCheck, Moon, Star, ChevronDown, ChevronUp, Timer, Lock, Phone, MessageCircle, ExternalLink,
  RefreshCw, Gamepad2
} from 'lucide-react';
...
import { doc, onSnapshot, collection, query, where, orderBy, limit, getDocs, setDoc, serverTimestamp, updateDoc, getDoc, clearIndexedDbPersistence } from 'firebase/firestore';
...
const EARNING_CATEGORIES = [
  ...
  { id: 'quiz', icon: Trophy, color: 'text-white', bg: 'bg-gradient-to-br from-yellow-400 to-orange-500' },
  ...
  return (
    <div className="absolute inset-0 pointer-events-none opacity-10">
      <div className="absolute bottom-0 left-0 w-full h-full">
        <svg viewBox="0 0 800 200" className="w-full h-full absolute bottom-0" preserveAspectRatio="none">
          <path d="M0,200 L0,150 C100,80 200,180 300,140 C400,100 500,180 600,130 C700,80 800,160 800,160 L800,200 Z" fill="currentColor" />
        </svg>
...
export function EarningView({ onBack }: EarningViewProps) {
  ...
  const [banners, setBanners] = useState<{ id: string | number; title: string; image: string; redirectLink?: string }[]>([]);

  useEffect(() => {
    const q = query(collection(db, 'ads'), where('active', '==', true), where('category', 'in', ['earning', 'all']));
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      if (fetchedBanners.length > 0) {
        setBanners(fetchedBanners.map(b => ({
          id: b.id,
          title: b.title || (language === 'bn' ? 'বিশেষ অফার' : 'Special Offer'),
          image: b.imageUrl,
          redirectLink: b.redirectLink
        })));
      } else {
        setBanners([
          { id: 1, title: language === 'bn' ? 'প্রতিদিন কুইজ খেলুন এবং জিতে নিন আকর্ষণীয় পুরস্কার!' : 'Play daily quiz and win attractive rewards!', image: 'https://picsum.photos/seed/promo1/900/300' },
          { id: 2, title: language === 'bn' ? 'বন্ধুদের রেফার করুন এবং বোনাস আয় করুন!' : 'Refer friends and earn bonus rewards!', image: 'https://picsum.photos/seed/promo2/900/300' },
          { id: 3, title: language === 'bn' ? 'নতুন মাইক্রো জবস এখন লাইভ - কাজ শুরু করুন!' : 'New Micro Jobs are now live - Start working!', image: 'https://picsum.photos/seed/promo3/900/300' },
          { id: 4, title: language === 'bn' ? 'আপনার ব্যালেন্স বিকাশ বা নগদে উইথড্র করুন সহজ উপায়ে' : 'Withdraw your balance easily via bKash or Nagad', image: 'https://picsum.photos/seed/promo4/900/300' },
        ]);
      }
    });
    return () => unsub();
  ...
  const handleAdClick = (redirectLink?: string) => {
    ...
    if (redirectLink.startsWith('https://wa.me/') || redirectLink.startsWith('tel:')) {
      ...
      // Regular URL
      window.open(redirectLink.startsWith('http') ? redirectLink : `https://${redirectLink}`, '_blank');
    }
  };
  ...
  useEffect(() => {
    ...
    const unsubBalance = onSnapshot(doc(db, 'user_balances', currentUser.uid), { includeMetadataChanges: true }, async (docSnap: any) => {
      if (docSnap.exists()) {
        const data = docSnap.data() as UserBalance;
        setBalance(data);
        
        if (docSnap.metadata.fromCache) {
          console.log("Balance loaded from cache, waiting for server sync...");
        }
  ...
  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      ...
      if (state && state.view && ['mobile-recharge', 'drive-offer', 'typing-job', 'micro-job', 'job-post', 'quiz-job', 'ad-view', 'deposit', 'withdraw', 'account-verification', 'terms', 'privacy'].includes(state.view)) {
  ...
  if (activeSubView === 'privacy') {
    return <LegalPageView type="privacy" onBack={() => setActiveSubView(null)} />;
  }
    ...
    <div className="min-h-screen bg-white dark:bg-slate-900 font-sans relative pb-0">
      ...
      <div className="relative bg-primary dark:bg-primary-dark pt-safe pt-2 pb-24 px-4 overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-3">
          ...
          <h1 className="text-xl font-black text-white flex-1">{t('earning')}</h1>
        ...
        <div className="bg-white dark:bg-slate-900 w-full rounded-t-xl shadow-xl shadow-black/5 overflow-hidden border-t border-slate-100 dark:border-slate-800 pb-0">
          ...
          <div className="p-4 flex items-center gap-4 bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-100 dark:border-slate-800">
            <div className="w-14 h-14 bg-white dark:bg-slate-900 rounded-full flex items-center justify-center p-0.5 shadow-sm border border-slate-200 dark:border-slate-700">
              <div className="w-full h-full rounded-full flex items-center justify-center overflow-hidden bg-primary/5">
                {(currentUser?.photoURL || (currentUser as any)?.user_metadata?.avatar_url || (currentUser as any)?.user_metadata?.picture) ? (
                  <img 
                    src={currentUser?.photoURL || (currentUser as any)?.user_metadata?.avatar_url || (currentUser as any)?.user_metadata?.picture} 
                    alt="Profile" 
            ...
            <div className="flex-1 flex flex-col gap-1 min-w-0">
              <h3 className="text-base font-black text-slate-900 dark:text-white leading-tight flex items-center gap-2 truncate">
                ...
                <VerifiedBadge isVerified={isVerified} isOwner={true} size={16} />
              </h3>
                ...
                {showBalance ? (
                  ...
                  >
                    <span className="text-[10px] font-black text-primary uppercase tracking-tighter">
                      {language === 'bn' ? 'ট্যাপ টু ব্যালেন্স' : 'Tap to Balance'}
                    </span>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <AnimatePresence mode="wait">
                {banners.length > 0 && banners[currentBanner] && (
                  <motion.div
                    ...
                    onClick={() => handleAdClick(banners[currentBanner].redirectLink)}
                  >
                    <img 
                    ...
                    <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-black/20 to-transparent flex items-center px-6">
                      <div className="max-w-[70%]">
                        ...
                        <button className="mt-2 bg-white text-primary text-[9px] font-black px-3 py-1 rounded-full shadow-lg active:scale-95 transition-transform flex items-center gap-1.5 leading-none">
                          {banners[currentBanner].redirectLink?.startsWith('tel:') || banners[currentBanner].redirectLink?.startsWith('https://wa.me/') 
                            ? (language === 'bn' ? 'যোগাযোগ করুন' : 'Contact Now')
                            : (language === 'bn' ? 'ট্যাপ করুন' : 'Tap Now')}
                          {banners[currentBanner].redirectLink?.startsWith('https://wa.me/') ? <MessageCircle className="w-2.5 h-2.5" /> : 
                           banners[currentBanner].redirectLink?.startsWith('tel:') ? <Phone className="w-2.5 h-2.5" /> : <ExternalLink className="w-2.5 h-2.5" />}
                        </button>
                      </div>
                    </div>
                  </motion.div>
          ...
          <div className="px-6 pb-6 pt-2">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-black text-slate-800 dark:text-slate-200 uppercase tracking-wider flex items-center gap-1.5">
                Community | {language === 'bn' ? 'প্রতিদিন 🪙 জিতুন' : 'Win 🪙 daily'}
              </h3>
            </div>
            
            {/* Banner Card */}
            <div className="bg-gradient-to-r from-yellow-100 to-amber-100 dark:from-yellow-900/30 dark:to-yellow-800/20 rounded-2xl p-6 relative overflow-hidden flex flex-col justify-between group cursor-pointer border border-yellow-200 dark:border-yellow-700/50 shadow-sm aspect-video" onClick={() => setActiveSubView('sudoku')}>
              ...
              <div className="absolute top-4 right-4 text-6xl opacity-40 drop-shadow-md rotate-12 transition-transform group-hover:rotate-6 group-hover:scale-110">🧩</div>
              ...
              <div className="relative z-10 flex flex-col gap-2 max-w-[65%]">
                ...
                <div>
                   <h4 className="text-slate-700 dark:text-slate-300 font-medium text-xs mb-0.5">{language === 'bn' ? 'খেলুন' : 'Play'}</h4>
                   <h3 className="text-2xl font-black text-slate-900 dark:text-white leading-tight uppercase tracking-wider">Sudoku<br/><span className="text-orange-600 dark:text-orange-500">Pro</span></h3>
                </div>
                <div className="text-[10px] font-bold text-slate-700 dark:text-slate-200 mt-2 bg-yellow-200/80 dark:bg-yellow-700/50 py-1.5 px-2.5 rounded-md self-start inline-flex items-center gap-1.5 border border-yellow-300 dark:border-yellow-600/50 shadow-sm">
                  {language === 'bn' ? 'প্রতিদিন জিতে নিন' : 'Win daily'} <span className="bg-orange-500 text-white rounded px-1.5 py-0.5 ml-1 inline-flex shadow-[0_2px_4px_rgb(249,115,22,0.3)]">১০০০ কয়েন</span>
                </div>
              </div>
              ...
              <div className="relative z-10 mt-6 flex justify-end">
                ...
                >
                  {language === 'bn' ? 'প্লে নাউ' : 'Play Now'} <ChevronRight className="w-4 h-4 ml-1" />
                </button>
              </div>
            </div>
          </div>
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          !isAnswered && "hover:border-primary/50 hover:bg-slate-50 text-slate-700 dark:text-slate-300",
                      ...
                      >
                        ...
                        {isAnswered && isCorrect && <CheckCircle2 className="w-5 h-5 text-emerald-500 relative z-10" />}
                        {isAnswered && isSelected && !isCorrect && <XCircle className="w-5 h-5 text-rose-500 relative z-10" />}
                      </button>
                    );
                  })}
                </div>
            ...
            >
              ...
              <div className="p-6 pt-safe mt-2 flex flex-col items-center border-b border-slate-100 dark:border-slate-800 relative overflow-hidden">
                ...
                <div className="w-full flex gap-2">
                  <div className="flex-1 flex flex-col items-center justify-center p-2.5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                    ...
                    <span className="text-sm font-black text-primary leading-none">৳{Number(balance?.currentBalance || 0).toFixed(2)}</span>
                  </div>
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center p-2.5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                    ...
                    <span className="text-sm font-black text-emerald-500 leading-none">৳{Number(balance?.depositBalance || 0).toFixed(2)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto py-4">
                ...
                <div className="px-4 pt-4 pb-4 space-y-1">
                  <div className="px-4 py-2">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      {language === 'bn' ? 'পলিসি ও শর্তাবলি' : 'Legal & Policy'}
                    </p>
                  </div>
                  ...
                  >
                    ...
                    <span className="text-sm">{language === 'bn' ? 'শর্তাবলি (T&C)' : 'Terms & Conditions'}</span>
                  </button>
                  ...
                  >
                    ...
                    <span className="text-sm">{language === 'bn' ? 'প্রাইভেসি পলিসি' : 'Privacy Policy'}</span>
                  </button>
                </div>
              </div>
